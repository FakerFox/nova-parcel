...
let current, showCompleted = false, toastTimer, contentKey;
...
function card(parcel) {
  ...
  return `<article class="parcel-card ${ready ? 'ready' : delivered ? 'delivered' : ''}">
    <div class="card-stage">${icon(ready ? 'box' : delivered ? 'check' : 'truck')}<span>${ready ? 'МОЖНА ЗАБИРАТИ' : delivered ? 'ОТРИМАНО' : 'У ДОРОЗІ'}</span><span class="direction" title="${parcel.direction === 'outgoing' ? 'Вихідна' : 'Вхідна'}">${parcel.direction === 'outgoing' ? '↗' : '↙'}</span></div>
...
function render(state) {
  ...
  const key = JSON.stringify([state.parcels, state.connected, state.awaitingLogin, state.error, state.notifications, showCompleted]);
  if (key !== contentKey) {
    contentKey = key;
    ...
    if (!state.parcels.length && !state.connected) {
      ...
      const active = state.parcels.filter(p => !['9','10','11'].includes(p.code));
      const visible = showCompleted ? state.parcels : active;
      $('content').innerHTML = `<div class="list-heading"><h1>Мої посилки</h1><span class="count">${active.length}</span><button class="icon-button" data-add aria-label="Додати ТТН" title="Додати ТТН">${icon('plus')}</button></div><div class="list-subheading"><span class="connection">${icon(state.connected ? 'shield' : 'box')}${state.connected ? 'Акаунт підключено' : 'Відстеження за ТТН'}</span><button id="show-completed" class="text-button">${showCompleted ? 'Лише активні' : 'Усі посилки'}</button></div>${state.error ? `<p class="error list-error" role="alert">${escape(state.error)}</p>` : ''}<div class="parcel-scroll">${visible.length ? visible.map(card).join('') : `<div class="empty">${icon('tray')}<strong>Активних посилок поки немає</strong><p>Нові відправлення з’являться<br>після наступної перевірки.</p></div>`}</div>${!state.notifications ? '<button id="enable-notifications" class="enable-notifications">Увімкнути сповіщення про зміни</button>' : ''}`;
...
if (!window.nova) {
  ...
  document.addEventListener('click', event => {
    ...
    if (event.target.closest('[data-add]')) { $('add-error').hidden = true; $('add-form').reset(); $('add-dialog').showModal(); }
  ...
  $('add-form').onsubmit = event => {
    ...
    run(async () => {
      const result = await api.add({ number:$('tracking-number').value, title:$('parcel-title').value });
      if (result.ok) $('add-dialog').close();
